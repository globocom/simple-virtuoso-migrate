...
class Main(object):
    ...
    def _load_triples(self):
        ...
        if os.path.isdir(files_to_load):
            files = [os.path.join(basepath, i) for i in os.listdir(files_to_load)]
        else:
            files = [files_to_load]

        files = [i for i in files if i.endswith('.ttl')]

        self._execution_log("- TTL(s) to upload: %r" % files, "GREEN", log_level_limit=1)
        ...
        if not self.config.get("show_sparql_only", False):
            ...
            if err_list:
                self._execution_log("ERRORS %r" % err_list, "RED", log_level_limit=1)

            if origen is None:
                origen = "insert"
    
            if ok_list:
                sparql_up, sparql_down = self.virtuoso.get_sparql(None, None, current_version, None, origen, ok_list)
                self._execute_migrations(sparql_up, sparql_down, current_version, current_version, out_list)
            else:
...
#
    #    current_version, origen = self.virtuoso.get_current_version()
    #    self._execution_log("- TTL to insert is: %s" % self.config.get("add_ttl"), "GREEN", log_level_limit=1)
#
    #    sparql_up, sparql_down = self.virtuoso.get_statements(self.config.get("add_ttl"), current_version, origen)
    #    self._execute_migrations(sparql_up, sparql_down, current_version, self.config.get("add_ttl"))
    ...
    def _migrate(self):
        ...
        if current_version is None:
            if self.config.get("file_migration") is not None:
                ...
                self._execution_log("- Destination version is: %s" % self.config.get("file_migration"), "GREEN", log_level_limit=1)
        ...
        if self.config.get("file_migration") is not None:
            ...
            destination_ontology = self.virtuoso.get_ontology_from_file(destination_version)
        else:
            destination_version = self._get_destination_version()
            destination_ontology = self.virtuoso.get_ontology_by_version(destination_version)

        sparql_up, sparql_down = self.virtuoso.get_sparql(current_ontology, destination_ontology, current_version, destination_version, source)
        self._execute_migrations(sparql_up, sparql_down, current_version, destination_version)
    ...
    def _get_destination_version(self):
        ...
        if destination_version is None:
            destination_version = self.virtuoso_migrate.latest_version_available()
        if destination_version is not None and not self.virtuoso_migrate.check_if_version_exists(destination_version):
            raise Exception("version not found (%s)" % destination_version)
        return destination_version

    def _execute_migrations(self, sparql_up, sparql_down, current_version, destination_version, out_list=None):
        self._execution_log("- Current version is: %s" % current_version, "GREEN", log_level_limit=1)
        self._execution_log("- Destination version is: %s" % destination_version, "GREEN", log_level_limit=1)
        ...
        if len(sparql_up.splitlines()) == 2 and self.config.get("load_ttl") is None:
            self._execution_log("\nNothing to do.\n", "PINK", log_level_limit=1)
            return
        ...
        if not self.config.get("show_sparql_only", False):
            ...
            self.virtuoso.execute_change(sparql_up, sparql_down, execution_log=self._execution_log)

        if self.config.get("show_sparql", False) or self.config.get("show_sparql_only", False):
            self._execution_log("__________ SPARQL statements executed __________", "YELLOW", log_level_limit=1)
    ...
    def _check_configuration(config):
        ...
        required_configs = ['database_host', 'database_endpoint', 'database_user', 'database_password', 'host_user', 'host_password', 'database_migrations_dir', 'database_port', 'database_graph', 'database_ontology', 'file_migration']
